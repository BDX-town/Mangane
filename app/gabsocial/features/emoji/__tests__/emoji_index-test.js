...
describe('emoji_index', () => {
  ...
  it('(different behavior from emoji-mart) do not erases custom emoji if not passed again', () => {
    ...
    expect(search('gabs').map(trimEmojis)).toEqual(lightExpected);
    expect(emojiIndex.search('gabs').map(trimEmojis)).toEqual(expected);
  });
  ...
  it('(different behavior from emoji-mart) erases custom emoji if another is passed', () => {
    ...
    emojiIndex.search('', { custom });
    const expected = [];
    expect(search('gabs', { custom: [] }).map(trimEmojis)).toEqual(expected);
    expect(emojiIndex.search('gabs').map(trimEmojis)).toEqual(expected);
  });
  ...
  it('handles custom emoji', () => {
    ...
    expect(search('gabs', { custom }).map(trimEmojis)).toEqual(expected);
    expect(emojiIndex.search('gabs', { custom }).map(trimEmojis)).toEqual(expected);
  });
