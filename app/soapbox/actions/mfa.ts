...
const fetchMfa = () =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState).get('/api/v1/pleroma/accounts/mfa').then(({ data }) => {
...
const fetchBackupCodes = () =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState).get('/api/v1/pleroma/accounts/mfa/backup_codes').then(({ data }) => {
...
const setupMfa = (method: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState).get(`/api/v1/pleroma/accounts/mfa/setup/${method}`).then(({ data }) => {
...
const confirmMfa = (method: string, code: string, password: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState).post(`/api/v1/pleroma/accounts/mfa/confirm/${method}`, params).then(({ data }) => {
...
const disableMfa = (method: string, password: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState).delete(`/api/v1/pleroma/accounts/mfa/${method}`, { data: { password } }).then(({ data }) => {
