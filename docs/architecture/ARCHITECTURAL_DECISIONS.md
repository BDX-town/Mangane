...
## ADR-028 — Entity Resolution & Creator Attribution share one canonical entity authority

Status: Accepted

Date: 2026-07-29

Decision: Replace the creator-only Phase 8B plan with an application-wide Entity Resolution & Creator Attribution phase. Mangane owns canonical entity IDs, evidence, aliases, relationships, mentions, resolutions, merge/split history, and account-safe derived bindings. Wikidata and DBpedia are optional enrichment and candidate providers, not ownership or truth authorities. Creator attribution, semantic hashtags, Custom Feeds, Search, Explore, Gist, recommendations, topic/entity pages, composer context, and the interpolator reuse this authority. Phase 15 adds retrieval and ranking over it rather than creating a second entity graph.
